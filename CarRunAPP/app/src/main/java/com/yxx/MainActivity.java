...
import com.examapp.AlarmActivity;
import com.examapp.CreateRealQuestionHelper;
import com.examapp.FindFragment;
import com.examapp.FirstFragment;
import com.examapp.Information;
import com.examapp.InformationDbHelper;
import com.examapp.MeFragment;
import com.examapp.PracticeFragment;
import com.examapp.RealQuestion;
import com.examapp.SchoolDbHelper;
import com.examapp.WarningActivity;
...
public class MainActivity extends AppCompatActivity {
    public List<com.examapp.Information>informationList1=new ArrayList<>();
    public List<com.examapp.schoolName>schoolNameList1=new ArrayList<>();
    ...
    private void loadData() {
            InformationDbHelper dbHelper=new InformationDbHelper(this,"inmessage.db",null,1);
            ...
            if (cursor.moveToFirst()) {
                do {
                 ...
                 com.examapp.Information information=new Information(id,title,content);
        ...
        private void upData(){
            InformationDbHelper informationDbHelper=new InformationDbHelper(this,"inmessage.db",null,1);
            SQLiteDatabase db= informationDbHelper.getWritableDatabase();
            ContentValues values=new ContentValues();
            values.put("id",1);
            values.put("title","指南");
            values.put("content","重要通知！江苏省2022年全国硕士研究生招生考试考生健康应试须知");
            db.insert("information",null,values);
            values.clear();

            values.put("title","实时资讯");
            values.put("id",2);
            values.put("content","考研后身价倍涨的5大专业，过程很难熬，但考上前途不用愁");
            db.insert("information",null,values);
            values.clear();

            values.put("title","实时资讯");
            values.put("content","考研前30天，他们这样学");
            values.put("id",3);
            db.insert("information",null,values);
            values.clear();

            values.put("title","实时资讯");
            values.put("content","研招网：2022年考研准考证打印时间入口以及打印教程");
            values.put("id",4);
            db.insert("information",null,values);
            values.clear();

            values.put("title","实时资讯");
            values.put("content","专科生考MBA，会被歧视？");
            values.put("id",5);
            db.insert("information",null,values);
            values.clear();

            values.put("title","实时资讯");
            values.put("content","申请MBA，你的理由是？");
            values.put("id",6);
            db.insert("information",null,values);
...
*/
    ...
    private void loadSchoolData() {
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                com.examapp.schoolName schoolNames=new schoolName(schoolid,name);
   ...
   /* private void loadBasicData() {
        CreateDbHelper createDbHelper=new CreateDbHelper(this,"myInfo.db",null,1);
